...
public class SingleArtistXiamiSpider {

	static Logger logger = LoggerFactory.getLogger(SingleArtistXiamiSpider.class);
	...
	public SingleArtistXiamiSpider(int xmid) {
		this.artist_id = xmid;
		xiamiFile = path + "xiamisong_artist_" + artist_id + ".txt";
	...
	public static void main(String args[]) throws IOException {
		...
		if (args != null && args.length > 0) {
			xmid = Integer.parseInt(args[0]);
		}

		SingleArtistXiamiSpider sp = new SingleArtistXiamiSpider(xmid);
		if (args.length > 2) {
	...
	public void start() throws IOException {
		...
		for (int hoti = 1; hoti < 6; hoti++) {
			String hotListUrl = "http://www.xiami.com/artist/top/id/" + artist_id + "/page/" + hoti;
			...
			if (songLineEls != null && songLineEls.size() > 0) {
				for (Element songLineEl : songLineEls) {
					...
					song.setXiamiArtistLink("/artist/" + artist_id);
